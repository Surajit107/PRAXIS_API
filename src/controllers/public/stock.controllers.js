...
const getStocks = asyncHandler(async (req, res) => {
  ...
  const nseStocksJson = await listPayloads(COLLECTION);
...
const getStockById = asyncHandler(async (req, res) => {
  ...
  const stock = await getPayloadByDocIdIlike(COLLECTION, stockSymbol);
  if (!stock) {
...
const getARandomStock = asyncHandler(async (req, res) => {
  const stock = await getRandomPayload(COLLECTION);
  if (!stock) {
    throw new ApiError(404, "Stock does not exist.");
  }
