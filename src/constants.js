...
/**
 * @type {{ UNKNOWN:"UNKNOWN"; RAZORPAY: "RAZORPAY"; PAYPAL: "PAYPAL"; STRIPE: "STRIPE"; } as const}
 */
...
/**
 * Prefer `DEFAULT_PAYMENT_PROVIDER` from env (stripe | razorpay | paypal).
 * Falls back to Stripe. Does not gate other providers — all remain usable when keyed.
 * @returns {typeof PaymentProviderEnum[keyof typeof PaymentProviderEnum]}
 */
export const getDefaultPaymentProvider = () => {
  const raw = String(process.env.DEFAULT_PAYMENT_PROVIDER || "STRIPE")
    .trim()
    .toUpperCase();
  if (
    AvailablePaymentProviders.includes(raw) &&
    raw !== PaymentProviderEnum.UNKNOWN
  ) {
    return raw;
  }
